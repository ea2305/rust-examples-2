...
pub struct Wall {
  pub size: f64,
  pub nodes: Vec<block::Block>
}

impl Wall {
  pub fn new (size: f64, nodes: Vec<block::Block>) -> Wall {
    Wall {
      size,
      nodes
    }
  ...
  pub fn craft (&mut self) {
    for i in 0..(self.size as u32) {
      ...
      // generate x top
      let x_top = block::Block { x: index, y: 0.0 };
      // generate x bottom
      let x_bottom = block::Block { x: index, y: self.size - 1.0 };
      // generate y left 
      let y_left = block::Block { x: 0.0, y: index };
      // generate y right     
      let y_right = block::Block { x: self.size - 1.0, y: index };
